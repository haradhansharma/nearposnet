...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_posapp_id'])) {
				$url .= '&filter_posapp_id=' . $this->request->get['filter_posapp_id'];
			}
...
/////sharma for pos
			if(isset($this->session->data['posapp_id'])){
				$session_posapp_id = $this->session->data['posapp_id'];
			}else{
				$session_posapp_id = 0;
			}
			if(isset($this->request->get['product_id'])){
				$product_posapp_id = $this->cart->getProductPosapp($this->request->get['product_id']);
			}
			if( $session_posapp_id == $product_posapp_id || $session_posapp_id ==0 ){
				$this->model_catalog_product->editProduct($this->request->get['product_id'], $this->request->post);
				$this->session->data['success'] = $this->language->get('text_success');
			}else{
				$this->response->redirect($this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . $url, true));
			...
			////sharma for pos TO GIVE PAYMENT FOR POS PRODUCT ENTRY DURING APPROVAL

			if($this->request->post['status'] == 1 ){
				$agent_code = $this->cart->getAgentOfProduct($this->request->get['product_id']);
				$posapp_id = $this->cart->getProductPosapp($this->request->get['product_id']);
				if($agent_code && $posapp_id){
					$data['entry_product_by_agent'] = (int)$this->cart->getEntryProductByAgent($agent_code, $posapp_id);
					$data['target_product_to_get_paid'] = (int)$this->config->get('module_posapp_target_product_to_pay');
					$data['customer_id'] = $this->cart->getCustomerIdOfAgent($agent_code);
					$data['balance_to_be_paid'] = round((int)$this->config->get('module_posapp_target_product_to_pay') - (int)$this->cart->getEntryProductByAgent($agent_code, $posapp_id), 2);

					if($data['entry_product_by_agent'] < $data['target_product_to_get_paid'] && !$this->cart->getTargetComplete($data['customer_id'], $posapp_id) ){						
						$data['pay'] = 0;
						$data['description'] = sprintf($this->language->get('text_description'), $data['entry_product_by_agent'] , $data['balance_to_be_paid'], $posapp_id);					
						$this->session->data['success'] = $data['description'];
						$this->model_catalog_product->addTransectionRecord($data);					
					}elseif((int)$data['balance_to_be_paid'] == 0 && !$this->cart->getTargetComplete($data['customer_id'])){					
						$data['pay'] = $this->config->get('module_posapp_agent_commission');
						$data['description'] = 'TargetComplete'. $posapp_id;					
						$this->session->data['success'] = $data['description'];
						$this->model_catalog_product->addTransectionRecord($data);
					}else{
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
          ...
          ////sharma for pos below is original
			// if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			if(isset($this->request->post['selected'])){
				foreach ($this->request->post['selected'] as $selected_product_id){				
					$product_posapp_id = $this->cart->getProductPosapp($selected_product_id);					
					if($product_posapp_id == $session_posapp_id || $session_posapp_id == 0 ){
						$this->model_catalog_product->deleteProduct($selected_product_id);
						$this->session->data['success'] = $this->language->get('text_success');
					}else{
						$this->response->redirect($this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . $url, true));
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			...
			if(isset($this->request->post['selected'])){
				foreach ($this->request->post['selected'] as $selected_product_id){				
					$product_posapp_id = $this->cart->getProductPosapp($selected_product_id);					
					if($product_posapp_id == $session_posapp_id || $session_posapp_id == 0){
						$this->model_catalog_product->copyProduct($selected_product_id);
						$this->session->data['success'] = $this->language->get('text_success');
					}else{
						$this->response->redirect($this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . $url, true));
	...
	protected function getList() {
		////sharma for pos
		if (isset($this->request->get['filter_posapp_id'])) {
			$filter_posapp_id = $this->request->get['filter_posapp_id'];
		} else {
			$filter_posapp_id = '';
		}
		if (isset($this->request->get['filter_name'])) {
		...
		$filter_data = array(
			////sharma for pos
			'filter_posapp_id'	  => $filter_posapp_id,
        ...
        ////sharma for pos
		$data['sort_posapp_id'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.posapp_id' . $url, true);
		...
		///sharma for pos
		$data['filter_posapp_id'] = $filter_posapp_id;
	...
	protected function getForm() {
		...
		if (isset($this->request->post['posapp_id'])) {
			$data['posapp_id'] = $this->request->post['posapp_id'];
		} elseif (!empty($product_info)) {
			$data['posapp_id'] = $product_info['posapp_id'];
		} else {
			$data['posapp_id'] = $posapp_id;
		}
		////sharma for pos
