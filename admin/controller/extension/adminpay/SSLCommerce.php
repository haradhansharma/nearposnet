...
class ControllerExtensionAdminpaySSLCommerce extends Controller {
	...
	public function index() {
		...
		$data['tran_id'] = $commission_order_id;
		$data['total_amount'] = $this->currency->format($order_info['payable'], $order_info['currency_code'], $order_info['currency_value'], false);
		...
		$data['success_url'] = $this->url->link('extension/payment/adminpay/SSLCommerce/callback', '', 'SSL');
        $data['fail_url'] = $this->url->link('extension/payment/adminpay/SSLCommerce/Failed', '', 'SSL');
        $data['cancel_url'] = $this->url->link('extension/payment/adminpay/SSLCommerce/Cancelled', '', 'SSL');
		...
		foreach ($this->model_possetting_commission_report->getCommissionOrderDetails($commission_order_id) as $product) {
    		$products .= 1 . ' x ' . $product['order_id']  .', ';
    	}		
		...
		if($this->config->get('payment_SSLCommerce_test')=='live') {
				$data['process_url'] = $this->url->link('extension/payment/adminpay/SSLCommerce/sendrequest', '', 'SSL');
				$data['api_type'] = "NO";
			}
		else {
				$data['process_url'] = $this->url->link('extension/payment/adminpay/SSLCommerce/sendrequest', '', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/adminpay/SSLCommerce')) {
			return $this->load->view($this->config->get('config_template') . '/template/extension/payment/adminpay/SSLCommerce', $data);
		} else {
			return $this->load->view('extension/payment/adminpay/SSLCommerce', $data);
		}
	}
	...
	public function sendrequest(){
        ...
        $data['fail_url'] = $this->url->link('extension/payment/adminpay/SSLCommerce/Failed', '', 'SSL');
        $data['cancel_url'] = $this->url->link('extension/payment/adminpay/SSLCommerce/Cancelled', '', 'SSL');
        $data['ipn_url'] = $this->url->link('extension/payment/adminpay/SSLCommerce/sslcommerz_ipn', '', 'SSL');
	...
	public function callback() 
	{
		...
		// $this->load->model('checkout/order');
		$this->load->model('possetting/commission_report');
		$order_info =  $this->model_possetting_commission_report->getCommissionOrder($order_id);
        $amount = $this->currency->format($order_info['payable'], $order_info['currency_code'], $order_info['currency_value'], false);
        ...
        $data['breadcrumbs'] = array();
			...
			$this->language->load('extension/payment/adminpay/SSLCommerce');
