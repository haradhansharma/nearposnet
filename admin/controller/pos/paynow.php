...
class ControllerPosPaynow extends Controller {
	public function index() {
		...
		$data['split_status'] = $this->config->get('setting_splitpayment');;
		$data['entry_payable_amount'] = $this->language->get('entry_payable_amount');
		$data['split_totals'] = $this->currency->format($this->pos->getTotal(), $this->config->get('config_currency'));
		// Customer Add
		...
		$data['text_partial_amount']=$this->language->get('text_partial_amount');
		$data['partial_status']=$this->config->get('setting_partial_payment');
		$partial_sum=$this->model_pos_pos->getSumPartialpayments($data['order_id']);
		if($total==$partial_sum){
			$data['partialsum_stats']=true;
		}else{
			$data['partialsum_stats']=false;
		}
		// new code 13 june 2020 //
		// Order Status
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['customer_name'])) {
			...
			foreach ($results as $result) {
				...
				$display = $result['name'].' '.$display_info;
	...
	public function addorder() {
		...
		if($this->request->post){
			...
			else if($this->request->post['payment_method']=='Partpayment')
			{
				if($this->request->post['partial_amount'] > $this->request->post['total']){
					$json['error']='Partial Amount must be less than total amount!';
				}
				else if(empty($this->request->post['order_status_id'])){
			...
			if ($this->request->post['payment_method']=='split') {
				$split_total = 0;
				$split_payments = array();
				$split_payment 	= $this->request->post['split_payment'];
				if(!empty($this->request->post['split_payment'])) {
					foreach($split_payment as $split_payment) {
						$split_total += $split_payment['amount'];
						$split_payments[] = array(
							'amount' 	 => $split_payment['amount'],
							'method' 	 => $split_payment['method'],
						);
					}
				}

				if($split_total < $this->pos->getTotal()) {
			...
			/// Total end
				...
				///edited by sharma for pos
				if($this->session->data['posapp_id'] <> 0 ){
					$order_data['store_name'] = $this->cart->getPosname($this->session->data['posapp_id']);
				}else{
				...
				if($this->session->data['posapp_id'] <> 0 ){					
					...
					$pos_url = $store_url . 'index.php?route=product/allpos/info' . '&posapp_id=' . $this->session->data['posapp_id'] ;

					$order_data['store_url'] = $pos_url;				
			...
			// Gift Voucher
				...
				{
					$order_data['partial_amount'] =$this->request->post['partial_amount'];
				}else{
					$order_data['partial_amount']='';
				}
			...
			/// split payment work 12-02-2020 //
				$order_data['split_payments'] = array();
				if(!empty($this->request->post['split_payment'])) {
					$order_data['split_payment'] 	= $this->request->post['split_payment'];
					foreach($order_data['split_payment'] as $split_payment) {
						$order_data['split_payments'][] = array(
							'amount' 	 => $split_payment['amount'],
							'method' 	 => $split_payment['method'],
						);
