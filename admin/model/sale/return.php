...
class ModelSaleReturn extends Model {
	...
	public function getReturns($data = array()) {
		...
		if($this->session->data['posapp_id'] > 0){
			if ($implode) {
				$sql .= " WHERE r.posapp_id = '". (int)$this->session->data['posapp_id'] ."' AND " . implode(" AND ", $implode);
			}else{
				$sql .= " WHERE r.posapp_id = '". (int)$this->session->data['posapp_id'] ."' ";
