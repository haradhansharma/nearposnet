...
class ModelExtensionDashboardSale extends Model {
	public function getTotalSales($data = array()) {
		...
		if($this->session->data['posapp_id'] > 0){
			$sql = "SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE posapp_id = '". $this->session->data['posapp_id'] ."' AND order_status_id > '0'";
		}else{
	...
	public function getTotalOrdersByCountry() {
		////sharma for pos
		if($this->session->data['posapp_id'] > 0){
			$query = $this->db->query("SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "country` c ON (o.payment_country_id = c.country_id) WHERE o.posapp_id = '". $this->session->data['posapp_id'] ."' AND o.order_status_id > '0' GROUP BY o.payment_country_id");
		}else{
	...
	public function getTotalOrdersByDay() {
		...
		if($this->session->data['posapp_id'] > 0){
			$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "order` WHERE posapp_id = '". $this->session->data['posapp_id'] ."' AND order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) = DATE(NOW()) GROUP BY HOUR(date_added) ORDER BY date_added ASC");
		}else{
	...
	public function getTotalOrdersByWeek() {
		...
		////sharma for pos
		if($this->session->data['posapp_id'] > 0){
			$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE posapp_id = '". $this->session->data['posapp_id'] ."' AND order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') GROUP BY DAYNAME(date_added)");
		}else{
	...
	public function getTotalOrdersByMonth() {
		...
		if($this->session->data['posapp_id'] > 0){
			$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE posapp_id = '". $this->session->data['posapp_id'] ."' AND order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(date_added)");
	...
	public function getTotalOrdersByYear() {
		...
		if($this->session->data['posapp_id'] > 0){
			$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` WHERE posapp_id = '". $this->session->data['posapp_id'] ."' AND order_status_id IN(" . implode(",", $implode) . ") AND YEAR(date_added) = YEAR(NOW()) GROUP BY MONTH(date_added)");
		}else{
	...
	public function getOrders($data = array()) {
		...
		if($this->session->data['posapp_id'] > 0){
			if (!empty($data['filter_order_status_id'])) {
				$sql .= " WHERE posapp_id = '". $this->session->date['posapp_id'] ."' AND order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
			} else {
				$sql .= " WHERE posapp_id = '". $this->session->date['posapp_id'] ."' AND order_status_id > '0'";
			}
	...
	public function getTaxes($data = array()) {
		...
		if($this->session->data['posapp_id'] > 0){
			if (!empty($data['filter_order_status_id'])) {
				$sql .= "AND o.posapp_id = '". $this->session->date['posapp_id'] ."' AND o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
			} else {
				$sql .= "AND o.posapp_id = '". $this->session->date['posapp_id'] ."' AND o.order_status_id > '0'";
			}
